...
const appConfig = defineConfig({
  html: {
    ...
    tags: [
      ...SINGLE_FILE_BUILD ? [] : [{
        tag: 'link',
        attrs: { rel: 'manifest', crossorigin: 'anonymous', href: 'manifest.json' }
      }],
      {
        tag: 'link',
        attrs: { rel: 'favicon', href: faviconPath }
      },
      ...SINGLE_FILE_BUILD ? [] : [{
        tag: 'link',
        attrs: { rel: 'icon', type: 'image/png', href: faviconPath }
      }, {
        tag: 'meta',
        attrs: { property: 'og:image', content: faviconPath }
      }]
    ]
  },
  output: {
  ...
  source: {
    ...
    define: {
      ...
      'process.env.GITHUB_URL': JSON.stringify(`https://github.com/${process.env.GITHUB_REPOSITORY || `${process.env.VERCEL_GIT_REPO_OWNER}/${process.env.VERCEL_GIT_REPO_SLUG}` || githubRepositoryFallback}`),
